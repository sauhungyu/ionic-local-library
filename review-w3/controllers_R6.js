...
.controller('MenuController', ['$scope', 'dishes', 'favoriteFactory', 'baseURL', '$ionicListDelegate', function($scope, dishes, favoriteFactory, baseURL, $ionicListDelegate) {
        ...
        .controller('FeedbackController', ['$scope', 'feedbackFactory', function($scope, feedbackFactory) {
            ...
            $scope.sendFeedback = function() {
                ...
                if ($scope.myfeedback.agree && ($scope.myfeedback.mychannel == "")) {
                    ...
                    $scope.invalidChannelSelection = false;
                    feedbackFactory.getFeedback().update($scope.myfeedback);
                    $scope.myfeedback = {mychannel:"", firstName:"", lastName:"", agree:false, email:"" };
        ...
        .controller('DishDetailController', ['$scope', '$stateParams', 'dish', 'menuFactory', 'favoriteFactory', 'baseURL', '$ionicPopover', '$ionicModal', function ($scope, $stateParams, dish, menuFactory, favoriteFactory, baseURL, $ionicPopover, $ionicModal) {
            ...
            $scope.dish.id = parseInt($stateParams.id,10);
            //Create the detail popover that we will use later
            ...
            //Triggered in the detail popover modal to close it
            $scope.closePopover = function() {
                $scope.detailPopover.hide();
            };

            //Open the detail popover modal
            ...
            //Perform the dish-comment action when the user submits the comment form
            $scope.doDishComment = function() {
                ...
                //Simulate a dish-comment delay. Remove this and replace with your dish-comment
                //code if using a server system
                $timeout(function() {
        ...
        .controller('FavoritesController', ['$scope', 'dishes', 'favorites', 'favoriteFactory', 'baseURL', '$ionicListDelegate', '$ionicPopup', '$ionicLoading', '$timeout', function ($scope, dishes, favorites, favoriteFactory, baseURL, $ionicListDelegate, $ionicPopup, $ionicLoading, $timeout) {
            ...
            $scope.deleteFavorite = function (index) {
                ...
                confirmPopup.then(function (res) {
                    if (res) {
                        console.log('Ok to delte');
                        favoriteFactory.deleteFromFavorites(index);
                    }else {
                        console.log('Canceled to delete');
                    }
                });
