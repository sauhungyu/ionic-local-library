  ...
  // Perform the reserve action when the user submits the reserve form
    ...
    .controller('MenuController', ['$scope','dishes', 'menuFactory', 'favoriteFactory', 'baseURL', '$ionicListDelegate', function ($scope, dishes, menuFactory, favoriteFactory, baseURL, $ionicListDelegate) {
	    ...
	    $ionicModal.fromTemplateUrl('templates/dish-comment.html', {
            ...
            $scope.openComment = popover;
        });
		$scope.addComment = {rating: 5, comment: "", author: "", date: ""};
            
        $scope.submitComment = function () {
                
            $scope.addComment.date = new Date().toISOString();
            console.log($scope.addComment);
                
            $scope.dish.comments.push($scope.addComment);
            menuFactory.update({id: $scope.dish.id}, $scope.dish);
            $scope.addComment = {rating: 5, comment: "", author: "", date: ""};
        };
        $scope.closeComment = function () {
            $scope.openComment.hide();
        };
